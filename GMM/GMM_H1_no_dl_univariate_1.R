...
#---------------------------------------------------------------------------------------------
#Function to compute bias correction - Exact Method (No drift No leverage)
#---------------------------------------------------------------------------------------------

bias_no_drift_lev = function(xi_true,lambda_true,nu_true,H_true,n=78){
  
  
  bias_no_df_lv = ((4*xi_true^2)/n) * integrate(function(y) {
    sapply(y, function(y) {
      (1-y)*exp((nu_true^2)/(2*lambda_true^(2*H_true))*(0.5*integrate(function(x) 
        exp(-abs(x))*abs(lambda_true*(y/n) + x)^(2*H_true), -Inf, Inf)$value 
        - abs(lambda_true*(y/n))^(2*H_true)))
    })
  }, 0, 1)$value 
  
  
  return(bias_no_df_lv)
...
#---------------------------------------------------------------------------------------------
 ...
 g1 <- function(tet,x)
 {
   ...
   m2 <- (IV_acov(tet[1],tet[2],tet[3],tet[4],0)+
            bias_no_drift_lev(tet[1],tet[2],tet[3],tet[4]) - x^2)#RV_acov$acf[1])
...
GMM_estimation <-function(RV){
  ...
  res_bias_no_dl <- gmm(g1,RV,c(xi_1 = initial_vals[1],lambda_1 = initial_vals[2],
                          nu_1=initial_vals[3],H_1 =initial_vals[4]), optfct = 'nlminb', 
                          type = "iterative", kernel = "Parzen",approx = "ARMA(1,1)",
                          lower =c(0.01,0.009,0.7,0.07), upper=c(0.03,0.02,0.8,0.15),itermax = 1000)

  summ_res_bias_no_dl = summary(res_bias_no_dl )
  
  #print(summ_res_bias_no_dl)
  return(c(initial_vals,summ_res_bias_no_dl$coefficients[,1],
           as.list(summ_res_bias_no_dl$stest[2]$test)[[1]]))
}
...
RV_daily = read.csv('RV1_1000_daily_1.csv',nrows=10) 
...
df_bias_no_dl = as.data.frame(t(apply(RV_daily, 1, function(x) GMM_estimation(RV = as.numeric(x)) )))
colnames(df_bias_no_dl)=c("xi0","lamda0","nu0","H0","xi", "lambda", "nu","H","J")
...
df_bias_no_dl[nrow(df_bias_no_dl) + 1,] = colMeans(df_bias_no_dl)
df_bias_no_dl[nrow(df_bias_no_dl) + 1,] = sapply(df_bias_no_dl,sd)
write.csv(df_bias_no_dl,file='GMM_H1_univariate_no_dl_1.csv', row.names=FALSE)
