...
RV_daily = read.csv('RV1_1000_daily_1.csv',nrows=2) 
...
df_bias_clt = as.data.frame(t(apply(RV_daily, 1, function(x) GMM_estimation(RV = as.numeric(x)) )))
colnames(df_bias_clt)=c("xi0","lamda0","nu0","H0","xi", "lambda", "nu","H","J")
...
write.csv(df_bias_clt,file='GMM_H1_univariate_clt_1.csv', row.names=FALSE)
